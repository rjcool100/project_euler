...
int grid[20][20]= {
		...
		{78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 4, 62, 16, 14, 90, 53, 56, 92},
...
int diagonalProduct(){ 
		...
		int product = 0;
		int temp;
		while(i <= gridlength - 4){ 
			for (int j = 0; j < gridlength - 4; ++j){ 
				temp = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3];
				if(temp >= product){
					product = temp;
				}
			}
			for(int j = 3; j < gridlength; ++j){ 
				temp = grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3];
				if(temp >= product){
					product = temp;
				}
...
int main(void) {
  //calculate product at horizontal, vertical and diagonals and comparing
    int diagonal=diagonalProduct();
    int vertical=verticalProduct();
    int horizontal=horizontalProduct();
   int product=0;
